...
export const generateMetadata = localePageMetadata("/venues", {
  title: "Venues",
  description:
...
type GroupKey = "uaePalaces" | "uaeUnique" | "uaeBeyond" | "international";

type GroupHeader = { eyebrow: string; title: string; description: string };

const groupVenues: Record<GroupKey, Venue[]> = {
  uaePalaces: [
    {
...
const groupOrder: GroupKey[] = ["uaePalaces", "uaeUnique", "uaeBeyond", "international"];
...
type Copy = {
  ...
  groups: Record<GroupKey, GroupHeader>;
};

// TODO(ru): review — drafted Russian copy pending principal sign-off.
const copy: Record<Locale, Copy> = {
  en: {
    heroEyebrow: "The Venues",
    heroTitle: "A private little black book.",
    heroDescription:
    ...
    guestsSuffix: "guests",
    footnoteLead:
      "Capacities are indicative ranges drawn from publicly available information and our own production notes. Every venue is reviewed in detail — access, acoustics, light, kitchen — before it appears on a couple's shortlist. The same book is opened for ",
    footnoteLinkLabel: "corporate & private commissions",
    groups: {
      uaePalaces: {
  ...
  ru: {
    heroEyebrow: "Площадки",
    heroTitle: "Маленькая приватная чёрная книжка.",
    heroDescription:
      "Объекты, на которых мы уже ставили, — и те, к которым возвращаемся. Вместимости и фирменные заметки ниже; полные досье открываются во второй беседе.",
    guestsSuffix: "гостей",
    footnoteLead:
      "Вместимости — ориентировочные диапазоны на основании открытой информации и наших продакшн-заметок. Каждая площадка детально изучается — подъезд, акустика, свет, кухня — прежде чем попасть в шортлист пары. Та же книга открывается для ",
    footnoteLinkLabel: "корпоративных и частных заказов",
    groups: {
      uaePalaces: {
        eyebrow: "ОАЭ · Дворцы и бальные залы",
        title: "Пятизвёздочные дома, в которых мы ставим чаще всего.",
        description:
          "Бальные залы отелей и дворцов в Дубае и Абу-Даби — выбранные за архитектуру, уровень сервиса и ту деликатность, которую они оказывают частному заказу.",
      },
      uaeUnique: {
        eyebrow: "ОАЭ · Необычные пространства",
        title: "Декорации за пределами привычного бального зала.",
        description:
          "Открытые, исторические и архитектурные площадки, к которым мы прибегаем, когда заказ требует того, чего глаз не ожидает.",
      },
      uaeBeyond: {
        eyebrow: "ОАЭ · За пределами Дубая",
        title: "Северные эмираты и природные декорации.",
        description:
          "Природные площадки для пар, ищущих тишину, горный воздух или пляж без линии небоскрёбов.",
      },
      international: {
        eyebrow: "Международные",
        title: "Объекты, к которым мы возвращаемся.",
        description:
          "Дома, дворцы и курорты за пределами Эмиратов, куда мы летим вместе с парой — остальная часть книги открывается во второй беседе.",
      },
    },
  },
};
      ...
      <Section theme="pearl">
        <Container>
          ...
          <p className="mt-24 max-w-2xl text-sm leading-relaxed text-ink/60">
            {t.footnoteLead}
            <Link href={{ pathname: "/services/[slug]", params: { slug: "corporate-and-private" } }} className="text-gilded-600 hover:text-gilded-800">
              {t.footnoteLinkLabel}
            </Link>
            .
          </p>
