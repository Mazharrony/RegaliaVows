...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <PhilosophyBlock
        heading={`Why ${item.shortTitle.toLowerCase()}.`}
        body={item.philosophy}
      />

      <SignaturesGrid
        heading="The signatures."
        intro="Four hallmarks that separate a Regalia Vows commission from the work of a producer-for-hire."
        items={item.signatures}
      />

      <ProcessTimeline
        heading="From first call to last farewell."
        steps={item.process}
      />
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr_1.3fr]">
            <Reveal>
              ...
              <p className="mt-6 max-w-sm text-base leading-relaxed text-ink/85">
                ...
                the foundations every client receives as standard.
              </p>
            </Reveal>
      ...
      {otherServices.length > 0 && (
        <Section theme="pearl" className="bg-cream-100">
          <Container>
            <Reveal>
              <Eyebrow className="!text-gilded-800">Continue With</Eyebrow>
